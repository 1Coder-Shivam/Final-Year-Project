            ...
            for (face in faces) {
                try {
                    ...
                    if (maskLabel == maskDetectionModel.NO_MASK) {
                        ...
                        if( !bestScoreUserName.equals("Unknown") && !set.contains(bestScoreUserName)){

                            set.add(bestScoreUserName)
                          //  fileText(set)
                            Log.d("number",set.toString())

                        }
                        Log.d("number",set.toString())
                        handeler.arengeSet(set)
                        predictions.add(
                            Prediction(
                                face.boundingBox,
                                bestScoreUserName
